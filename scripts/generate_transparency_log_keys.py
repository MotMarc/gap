...
def ensure_transparency_log_key_pair() -> None:
    private_exists = PRIVATE_KEY_PATH.exists()
    public_exists = PUBLIC_KEY_PATH.exists()
    if private_exists != public_exists:
        raise RuntimeError("Refusing to proceed: only one transparency log key exists.")
    if private_exists:
        ...
        if derived != configured:
            raise RuntimeError("Existing transparency log key pair does not match.")
        print("Valid transparency log key pair already exists; unchanged.")
        return
    private_key, public_key = generate_provider_key_pair()
    save_private_key(private_key, PRIVATE_KEY_PATH)
    save_public_key(public_key, PUBLIC_KEY_PATH)
    print("Generated transparency log Ed25519 key pair.")


if __name__ == "__main__":
    try:
        ensure_transparency_log_key_pair()
    except (OSError, ValueError, RuntimeError) as error:
        print(f"Transparency log key error: {error}", file=sys.stderr)
        raise SystemExit(1)
